...
def format_number(latest_serial,existing_serials,register,batch,batch_size):
    '''
        ...
        for i in range(num):
            if 0 <= i <= 9999:
                if register == True:
                    ...
                    while formatted_num in existing_serial_list:
                        i += 1
                        formatted_num = '{:04d}'.format(start_num+i)
...
def get_latest_serial(client,xxyy, production_status, N2, flavor, register,comp_type,batch,batch_size): 
    '''
    ...
    for serial in existing_serials:
        if len(serial) != 14:
            ...
            if latest_serial < int(serial[10:14]):
                ...
                pass
    new_serial = format_number(latest_serial,existing_serials,register,batch,batch_size)
